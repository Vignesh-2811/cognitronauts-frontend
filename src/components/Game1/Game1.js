...
function DrawMaze(Maze, ctz, cellsize, endSprite = null) {
var map = Maze.map();
var cellSize = cellsize;
var drawEndMethod;
ctz.lineWidth = cellSize / 40;

this.redrawMaze = function (size) {
cellSize = size;
ctz.lineWidth = cellSize / 50;
drawMap();
drawEndMethod();
};
...
if (cell.n == false) {
    ctz.beginPath();
    ctz.moveTo(x, y);
    ctz.lineTo(x + cellSize, y);
    ctz.stroke();
}
if (cell.s === false) {
    ctz.beginPath();
    ctz.moveTo(x, y + cellSize);
    ctz.lineTo(x + cellSize, y + cellSize);
    ctz.stroke();
}
if (cell.e === false) {
    ctz.beginPath();
    ctz.moveTo(x + cellSize, y);
    ctz.lineTo(x + cellSize, y + cellSize);
    ctz.stroke();
}
if (cell.w === false) {
...
for (let y = 0; y < gridSize; y++) {
    ...
    for (let x = 0; x < gridSize; x++) {
        ...
        if (colorSwap) {
            ctz.fillStyle = "rgba(0, 0, 0, 0.8)";
        } else {
            ctz.fillStyle = "rgba(255, 255, 255, 0.8)";
        }
        ctz.fill();
        colorSwap = !colorSwap;
    }
...
ctz.clearRect(0, 0, canvasSize, canvasSize);
}
...
var ctz = mazeCanvas.getContext("2d");
var sprite;
var finishSprite;
...
ctz.canvas.width = viewHeight - viewHeight / 100;
ctz.canvas.height = viewHeight - viewHeight / 100;
} else {
ctz.canvas.width = viewWidth - viewWidth / 100;
ctz.canvas.height = viewWidth - viewWidth / 100;
}
...
document.getElementById("mazeContainer").style.opacity = "100";
  ...
  return (
    ...
    {/* <style>
        ...
        #right-half {
            width: 50%;
            background-color: white; /* You can set the background color for the empty right half */
        }
